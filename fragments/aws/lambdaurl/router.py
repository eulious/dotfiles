...
class APIRouter:
    ...
    def __add(self, url: str, method: str):
        def _wrapper(func):
            ...
            self._add_route(self.__prefix + url, method, func)
            return wrapper
        return _wrapper

    def _add_route(self, url: str, method: str, func: function):
        print(self.__prefix + url, method, func)
        self._routes.append((self.__prefix + url, method, func))


class LambdaURL(APIRouter):
    ...
    def add_router(self, router: APIRouter):
        router.app = self
        for (url, method, func) in router._routes:
            self.__router.add(self.__prefix + url, method, func)
